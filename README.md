...
```mermaid
flowchart LR
    S(["npm run loop"]) --> T["TASKS.md から<br/>次タスクを取得"]
...
`.claude/loop/TASKS.md` にタスクが箇条書きで並んでいる。ループが先頭から1つずつ取り出して実装し、終わったら Claude が完了に移動する。
...
2. **タスクリスト** -- `.claude/loop/TASKS.md` の「次にやること」に、やりたいタスクを並べる
...
| [.claude/](.claude/) | ループ機構一式（エンジン・フック・チェック役・タスクリスト） |
